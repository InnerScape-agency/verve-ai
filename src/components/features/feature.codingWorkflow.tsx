...
import CodingWorkflowCard from './codingWorkflow.card'
import { codingWorkflows } from '@/constant/feature/codingWorkflow'

export default function FeatureCodingWorkflow() {
  return (
    <section
      style={{
    ...
    >
      <div className="container">
        <div className="flex items-start justify-around">
          {/* left */}
          <div className="sticky top-6 h-fit max-w-120">
            <SectionHeader
              alingMent="left"
              varient="light"
              semiTitle="Coding Workflow"
              title="How We Power Coding Interviews"
              svg={
                <svg
                  width="20"
                  height="20"
            ...
            <p className="font-inter text-dark-900 mt-5 max-w-[80%] text-lg leading-[170%]">
              From instant setup to live question capture and optimized solutions, our AI supports
              every step of your coding interview journey.
            </p>
            ...
            <button className="font-inter text-dark bg-primary mt-10 flex cursor-pointer items-center gap-1.5 rounded-lg px-6 py-4 text-base font-medium">
              Sign Up <ChevronRight width={16} height={16} />
            </button>
          </div>

          {/* right */}
          <div className="space-y-4">
            {codingWorkflows.map((data) => (
              <CodingWorkflowCard key={data.title} data={data} />
            ))}
          </div>
        </div>
      </div>
